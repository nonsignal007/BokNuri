...
model = Wav2Vec2ForCTC.from_pretrained("Taeham/wav2vec2-ksponspeech").to('mps')
...
input_values = tmp.input_values.to('mps')
with torch.no_grad():
    input_values = input_values.type(torch.FloatTensor).to('mps')
